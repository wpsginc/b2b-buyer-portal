import {
  ...
  function PaginationTable(
    ...
    const cacheList = useCallback(
      (orders: Array<CustomFieldItems>) => {
        if (!cacheAllList.length) setCacheAllList(orders)
  
        const copyCacheAllList = [...cacheAllList]
  
        orders.forEach((item: CustomFieldItems) => {
    ...
    const fetchList = useCallback(
      async (b3Pagination?: TablePagination, isRefresh?: boolean) => {
        try {
          ...
          const { orders, totalCount }: CustomFieldItems = requestList
  ...
  const CustomPaginationTable = memo(forwardRef(PaginationTable))
  
  export { CustomPaginationTable }
